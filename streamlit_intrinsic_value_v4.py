...
enable_ai_insights = st.sidebar.checkbox(
    ...
    help="Get AI-powered contextual insights using Groq API (free)"
)
...
if enable_ai_insights:
    ...
    if groq_api_key:
        ...
        st.sidebar.info("💡 Enter API key or set GROQ_API_KEY environment variable")
else:
    groq_api_key = None
...
if calculate_button:
    ...
    ai_insights = {}  # NEW: Store AI insights
    failed_tickers = []
    ...
    if enable_ai_insights and groq_api_key:
        try:
            ...
            if ai_generator.test_connection():
                st.info("🤖 AI insights enabled")
            else:
                st.warning("⚠️ AI connection test failed")
                ai_generator = None
    ...
    for idx, ticker in enumerate(tickers):
        ...
        try:
            ...
            # NEW: Generate AI insights if enabled
            if ai_generator and iv_values:
                try:
                    # Get company info for context
                    company_info = fetcher.get_info() if 'fetcher' in locals() else {}
    ...
    if results:
        ...
        if enable_ai_insights and ai_insights:
            ...
            st.markdown("*Contextual insights generated by Llama 3.1 via Groq API*")
            
            for ticker, insight in ai_insights.items():
                with st.expander(f"💡 {ticker} - AI Analysis", expanded=True):
                    st.markdown(insight)
                    st.caption("🔒 Powered by Groq (free tier) • Data processed securely")
